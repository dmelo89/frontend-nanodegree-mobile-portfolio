...
var resizePizzas = function(size) { 
  ...
  // Removed a function inside changePizzaSizes and made it only one function.
  // I simplified the code, removing unecessary "returns" and changing them to a value in a variable called newwidth. Instead of using pixels, now the funciton uses percentages 
  ...
  // But is not necessary for the users so I'm commenting it.
...
// I reduced the amount of pizzas generated. Too many flavours.
for (var i = 2; i < 40; i++) {
...
// var frame = 0; Removed because I removed pizzas from the background
...
var positionY = 0; // Initial position for Y

// The Scrolling Function uses requestAnimationFrame for better performace on rendering
function Scrolling() {
  positionY = window.scrollY; // Gets last position of the scroll
...
//  window.performance.mark("mark_start_frame");
  ...
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((positionY / 1250) + (i % 5));
    items[i].style.left = items[i].basicLeft + 100 * phase + 'px';
  }

  // User Timing API to the rescue again. Seriously, it's worth learning.
  // Super easy to create custom metrics.
  // Not useful for the User Experience, so commenting to not be in the compiled project
...
//
// NOTE: This shouldn't render pizzas vertically after the 'fold' the movement is only horizontal and the position is fixed.
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8; // Figured out 8 cols is sufficient for most resolutions
  var s = 256;
  for (var i = 0; i < 40; i++) { // Reduced number of pizzas because they would go away from the viewport
    var elem = document.createElement('div'); // changed to a div to remove the 'src' attribute
    elem.className = 'mover';
 //   elem.src = "images/pizza.png"; Removed this from code. I am using sizes and images in the CSS file.
